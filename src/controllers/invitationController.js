...
export const acceptInvite = async (inviteId, eventId, notification, remove, fetchEvent, userId) => { //TODO PERHAPS: add the remove/fetchEvent/authenticate function above and call it here
    ...
    const createEventResponse = await axios.post(`http://internal-api-service.default.svc.cluster.local:5050/api/users/createEvent`, {event: eventData, userId: userId, shared: []});
  ...
  try {
    await axios.delete(`http://internal-api-service.default.svc.cluster.local:5050/api/invites/invite/${inviteId}`);
...
export const fetchInvites = async (userId, setInvites) => {
  try {
    const response = await axios.get(`http://internal-api-service.default.svc.cluster.local:5050/api/invites/receivedInvites/${userId}`);
...
export const fetchInviterUsername = async (inviterId) => {
  try {
    const response = await axios.get(`http://internal-api-service.default.svc.cluster.local:5050/api/users/usernameFromId/${inviterId}`);
  ...
  try {
    for (let username of selectedUsers) {
      ...
      if (user) {
        ...
        await axios.post('http://internal-api-service.default.svc.cluster.local:5050/api/invites/createInvite', inviteData);
      }
      else 
      {
