...
export const fetchUsers = async () => {
    try {
        const response = await axios.get('http://internal-api-service.default.svc.cluster.local:5050/api/users');
...
export const loginUser = async (username, password, rememberMe) => {
    try {
        const response = await axios.post('http://internal-api-service.default.svc.cluster.local:5050/api/users/login', {
            username,
            password
        });
...
export const registerUser = async (formData) => {
    try {
        const response = await axios.post('http://internal-api-service.default.svc.cluster.local:5050/api/users/signup', formData);
        if (response.status === 200) {
...
export async function authenticate()
{
  ...
  } else
  {
    ...
    try {
    const response = await axios.post('http://internal-api-service.default.svc.cluster.local:5050/api/users/userId', {
